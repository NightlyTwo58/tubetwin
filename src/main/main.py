    ...
    while current_day < now:
        ...
        for ch in channel_data:
            ...
            try:
                ...
                sys.exit(f"[WARN] Fatal error fetching videos for {channel_id}: {e}")

            if video_ids is None:
                save_cache(cache, CACHE_JSON)
                sys.exit("[WARN] Quota hit during recent video fetch, stopping.")
            ...
            # ------------------ VIDEO STATS ------------------
            video_stats = get_video_stats(new_videos)
            ...
            for video in video_stats:
                ...
                if comments is None:
                    save_cache(cache, CACHE_JSON)
                    sys.exit(
                        f"[WARN] Quota hit during comments for video {video['video_id']}, stopping."
                    )
...
def write_comments_csv(rows):
    header = [
        "date", "channel_id", "channel_title", "channel_description", "channel_topics",
...
def write_channel_stats_csv(summaries):
    header = [
        "date", "channel_id", "channel_subs",
        "channel_total_views", "channel_video_count", "channel_cluster"
    ]
